#!/usr/bin/env python3
"""
Accuracy verification script for protein sequence model using BERT version ProteinGym database

This script uses a trained BERT protein sequence model to
...
class BERTProteinGymEvaluator(ModelEvaluator):
    """Model evaluation class using BERT version ProteinGym data"""
    ...
    def _init_tokenizer(self) -> Any:
        ...
        try:
            ...
            # Obtain special token ID (according to EsmSequenceTokenizer specifications)
            ...
            # What to do if vocab_size is different from expected
    ...
    def _init_simple_amino_acid_tokenizer(self) -> None:
        """Initialize a simple amino acid tokenizer (for fallback)"""
    ...
    def encode_sequence(self, sequence: str, max_length: int = 512) -> torch.Tensor:
        ...
        # What to do if tokens is not a list
        ...
        if len(tokens) > max_length - 2:  # Reserve two for CLS, SEP
            tokens = tokens[: max_length - 2]

        # Convert to BERT format: [CLS] + tokens + [SEP]
    ...
    def get_masked_lm_score(
        ...
        """
        Calculate mask language model score

        Args:
            sequence (str): amino acid sequence
            mutation_pos (int): mutation position (optional)
            context_length (int): Context length used for evaluation

        Returns:
            float: MLM score (average of log likelihood)
        """
        with torch.no_grad():
            # encode array
    ...
    def get_variant_fitness_score(
        ...
        """
        Calculate the fitness score of a mutation（BERTversion)
        ...
        """
        try:
            # Calculate MLM score for wild type and mutant type
            ...
            # Fitness score = mutant - wild type
            # Positive value = mutation is beneficial, negative value = mutation is harmful
            fitness_score = mut_score - wt_score

            # Also calculate the similarity of array representations
    ...
    def get_sequence_similarity(self, seq1: str, seq2: str, context_length: int = 512) -> float:
        ...
        with torch.no_grad():
            ...
            repr1 = hidden_states[0, 0, :]  # [CLS] of first array
            repr2 = hidden_states[1, 0, :]  # [CLS] of second array
    ...
    def _calculate_pathogenicity_score(self, fitness_score: float, similarity: float) -> float:
        """Calculate virulence score (range 0-1)"""
        # Compute virulence score by combining fitness score and similarity
        # Low fitness + low similarity = high pathogenicity

        # Normalize fitness score (using sigmoid function)
        fitness_norm = 1 / (1 + np.exp(fitness_score))

        # Reverse similarity (lower similarity = higher change)
        dissimilarity = 1 - similarity

        # weighted average(more weight on fitness score)
    ...
    def evaluate_model(
        ...
        for batch_index, i in enumerate(
            ...
            # Output at regular intervals so that progress is logged
            if batch_index == 1 or batch_index % 10 == 0 or batch_index == total_batches:
                logger.info(f"Batch progress: {batch_index}/{total_batches} ({batch_index / total_batches * 100:.1f}%)")

            batch = proteingym_data.iloc[i : i + batch_size]

            for idx, row in batch.iterrows():
                try:
                    # Obtain wild type and mutant sequences
                    if "target_seq" in row and pd.notna(row["target_seq"]):
                        wt_seq = row["target_seq"]
                        mut_seq = row["mutated_sequence"]
                    else:
                        # Processing when there is no target_seq
                    ...
                    if total_variants > 0:
                        ...
                        if percent != last_percent:
                            logger.info(
                                f"Step: evaluation Progress: {percent}% ({processed}/{total_variants} variants processed)"
                            )
...
def create_sample_proteingym_data(output_file: str) -> None:
    ...
    # Sample data (based on actual ProteinGym data format)
    sample_data = [
        {
            "mutant": "A1V",
...
def main() -> None:
    ...
    try:
        ...
        # protein_sequence uses EsmSequenceTokenizer, so tokenizer_path can be None.
