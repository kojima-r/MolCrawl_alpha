...
class GPT2ClinVarEvaluator(ModelEvaluator):
    ...
    def load_clinvar_data(self, clinvar_file):
        ...
        # 必要なカラムの確認
        required_columns = ['reference_sequence', 'variant_sequence', 'ClinicalSignificance']
    ...
    def evaluate_model(self, clinvar_data, batch_size=32):
        ...
        for i in range(0, len(clinvar_data), batch_size):
            ...
            for _, row in batch.iterrows():
                try:
                    # 病原性スコアを計算
                    score = self.get_variant_probability(
...
def main():
    ...
    parser.add_argument('--clinvar_data', type=str, required=True,
