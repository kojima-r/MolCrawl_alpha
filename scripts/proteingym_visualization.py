...
class ProteinGymVisualizer:
    ...
    def plot_correlation_scatter(self, true_scores, predicted_scores, title="Correlation Plot", save_name="correlation_scatter.png"):
        """
        相関散布図を作成
    ...
    def plot_score_distributions(self, true_scores, predicted_scores, save_name="score_distributions.png"):
        ...
        # ヒストグラム
        ax1.hist(true_scores, bins=30, alpha=0.7, label='True Scores', color='blue', density=True)
        ax1.hist(predicted_scores, bins=30, alpha=0.7, label='Predicted Scores', color='red', density=True)
        ...
        for bar, value, name in zip(bars, values, names):
            ...
            if name in ['MAE', 'RMSE']:
                ...
                ax.text(bar.get_x() + bar.get_width()/2., height,
                       f'{label_value:.4f}', ha='center', va='bottom' if height >= 0 else 'top')
            else:
                ax.text(bar.get_x() + bar.get_width()/2., height,
    ...
    def plot_mutation_type_analysis(self, df, save_name="mutation_analysis.png"):
        ...
        for i, mut_type in enumerate(mutation_types):
            ...
            ax1.scatter(subset['true_score'], subset['predicted_score'], 
                       label=f'{mut_type} (n={len(subset)})', alpha=0.6)
        ...
        ax4.bar(mutation_types, correlations, alpha=0.7, color=['blue', 'green', 'red'][:len(mutation_types)])
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='./proteingym_visualizations',
