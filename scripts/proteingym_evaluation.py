...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/proteingym_evaluation_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
...
class ProteinGymEvaluator:
    ...
    def __init__(self, model_path, tokenizer_path=None, device='cuda'):
        ...
        self._init_protein_tokenizer()
        
        # モデルの読み込み
        logger.info(f"Loading model from {model_path}")
        self.model = self._load_model()
    
    def _init_protein_tokenizer(self):
        """protein_sequence用のトークナイザーを初期化"""
        try:
    ...
    def _infer_target_sequence(self, df):
        ...
        df['target_seq'] = df.apply(lambda row: reverse_mutation(row['mutated_sequence'], row.get('mutant', '')), axis=1)
        return df
...
def main():
    ...
    parser.add_argument('--output_dir', type=str, default='./proteingym_evaluation_results',
    ...
    try:
        ...
        with open(report_file, 'w') as f:
            ...
            f.write(f"  True scores - mean: {results['true_score_stats']['mean']:.4f}, std: {results['true_score_stats']['std']:.4f}\n")
            f.write(f"  Predicted scores - mean: {results['predicted_score_stats']['mean']:.4f}, std: {results['predicted_score_stats']['std']:.4f}\n")
