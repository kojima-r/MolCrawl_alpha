...
BaseVisualizationGeneratorを継承してmolecule_nlに特化した
分析結果の可視化を行います。
"""
...
class MoleculeNLVisualizationGenerator(BaseVisualizationGenerator):
    ...
    def __init__(self, results_file, output_dir):
        ...
        self.evaluation_type = "molecule_nl"
    ...
    def create_domain_specific_plot(self):
        ...
        fig.suptitle('Molecule NL Model - Detailed Perplexity Analysis', fontsize=16, fontweight='bold')
        
        # 1. パープレキシティ分布（線形スケール）
        axes[0, 0].hist(self.df['perplexity'], bins=50, alpha=0.7, 
        ...
        # 2. パープレキシティ分布（対数スケール）
        axes[0, 1].hist(self.df['perplexity'], bins=50, alpha=0.7, 
        ...
        # 3. テキスト長とパープレキシティの関係
        axes[0, 2].scatter(self.df['text_length'], self.df['perplexity'], 
                          alpha=0.6, color=self.colors['info'], s=20)
        
        # 傾向線を追加
        z = np.polyfit(self.df['text_length'], self.df['perplexity'], 1)
        p = np.poly1d(z)
        axes[0, 2].plot(self.df['text_length'], p(self.df['text_length']), 
        ...
        # 4. トークン長とパープレキシティの関係
        axes[1, 0].scatter(self.df['token_length'], self.df['perplexity'], 
                          alpha=0.6, color=self.colors['accent'], s=20)
        
        # 傾向線を追加
        z_token = np.polyfit(self.df['token_length'], self.df['perplexity'], 1)
        p_token = np.poly1d(z_token)
        axes[1, 0].plot(self.df['token_length'], p_token(self.df['token_length']), 
                       "r--", alpha=0.8, linewidth=2)
        ...
        box_data = [self.df[self.df['length_quartile'] == q]['perplexity'].values 
                   for q in ['Q1', 'Q2', 'Q3', 'Q4']]
        
        box_plot = axes[1, 1].boxplot(box_data, labels=['Q1', 'Q2', 'Q3', 'Q4'], 
                                     patch_artist=True)
        
        # ボックスプロットの色設定
        colors_box = [self.colors['primary'], self.colors['secondary'], 
                     self.colors['info'], self.colors['accent']]
        ...
        range_counts = []
        range_labels = []
        range_colors = [self.colors['success'], self.colors['info'], 
        ...
        bars = axes[1, 2].bar(range(len(range_counts)), range_counts, 
                             color=range_colors, alpha=0.8, edgecolor='black')
        ...
        for bar, count in zip(bars, range_counts):
            ...
            axes[1, 2].text(bar.get_x() + bar.get_width()/2., height + max(range_counts)*0.01,
                           f'{count}', ha='center', va='bottom', fontweight='bold')
        
        plt.tight_layout()
        
        # ファイル保存
        output_path = self.output_dir / "molecule_nl_detailed_analysis.png"
    ...
    def generate_statistics_summary(self):
        ...
        stats = {
            ...
            'Performance Distribution': {
                ...
                'Good (10 ≤ PPL < 50)': len(self.df[(self.df['perplexity'] >= 10) & 
                                                   (self.df['perplexity'] < 50)]),
                'Moderate (50 ≤ PPL < 200)': len(self.df[(self.df['perplexity'] >= 50) & 
                                                        (self.df['perplexity'] < 200)]),
    ...
    def _create_comprehensive_evaluation_dashboard(self):
        ...
        perplexity_scores = self.df['perplexity'].values if hasattr(self, 'df') and self.df is not None else np.random.lognormal(2, 1, n_samples)
        # パープレキシティを0-1スケールのスコア（高いほど良い）に変換
        max_perplexity = np.percentile(perplexity_scores, 95)  # 外れ値を除外
        sample_data = {
            'y_true': np.random.choice([0, 1], size=n_samples, p=[0.3, 0.7]),  # 分子理解タスクの正解
            'y_pred': np.random.choice([0, 1], size=n_samples, p=[0.25, 0.75]),  # 予測結果
            'y_score': 1.0 - np.clip(perplexity_scores / max_perplexity, 0, 1),  # パープレキシティから導出したスコア
        ...
        dashboard_path = self._create_comprehensive_dashboard(sample_data)
    ...
    def create_html_report(self):
        ...
        # HTMLファイルの保存
        html_path = self.output_dir / "molecule_nl_evaluation_report.html"
