#!/bin/bash
# Fine-tune the compounds GPT-2 (large) model on ChEMBL.
#
# Prerequisites:
#   - compounds GPT-2 pretraining checkpoint must exist in
#       $LEARNING_SOURCE_DIR/compounds/gpt2-output/compounds-large/
...
#   bash workflows/03a-compounds_chembl-train-gpt2-large.sh
...
nohup bash -c '$PYTHON molcrawl/gpt2/train.py \
    gpt2/configs/compounds/train_gpt2_chembl_large.py' \
    > "${LOG_DIR}/compounds_chembl-train-gpt2-large-$(date +%Y-%m-%d_%H-%M-%S).log" 2>&1 &
