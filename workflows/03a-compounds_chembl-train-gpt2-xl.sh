#!/bin/bash
# Fine-tune the compounds GPT-2 (xl) model on ChEMBL.
#
# Prerequisites:
...
#   bash workflows/03a-compounds_chembl-train-gpt2-xl.sh
...
nohup bash -c '$PYTHON molcrawl/gpt2/train.py \
    ...
    > "${LOG_DIR}/compounds_chembl-train-gpt2-xl-$(date +%Y-%m-%d_%H-%M-%S).log" 2>&1 &
