...
class GPNEmbeddingWPosition(GPNEmbedding):
    ...
    def forward(self, input_ids=None, input_probs=None, aux_features=None):
        embeddings = super().forward(input_ids, input_probs, aux_features)
...
class GPNBerConfig(GPNRoFormerConfig):
    model_type = "GPNBert"


class GPNBertPreTrainedModel(PreTrainedModel):
    config_class = GPNBerConfig


class GPNBertModel(GPNBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        self.embedding = GPNEmbeddingWPosition(config)
...
class GPNBertForMaskedLM(GPNBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)

        self.model = GPNBertModel(config)
...
class GPNGPT2Config(GPT2Config, GPNRoFormerConfig):
    model_type = "GPNGPT2"


@dataclass
class CausalLMGPNOutputWithCrossAttentions(ModelOutput):
...
class GPNEmbeddingGPT2(GPNEmbedding):
    # weight = None
...
class GPNGPT2Model(GPT2Model):
    config_class = GPNGPT2Config

    def __init__(self, config):
        super().__init__(config)
        self.wte = GPNEmbeddingGPT2(config)


class GPNGPT2LMHeadModel(GPT2LMHeadModel):
    def tie_weights(self):
        # Override to skip tying
        pass
    ...
    def __init__(self, config):
        super().__init__(config)
        self.transformer = GPNGPT2Model(config)
    ...
    def forward(self, *args, **kwargs):
        ...
        return CausalLMGPNOutputWithCrossAttentions(
            loss=output.loss,
...
AutoConfig.register("GPNBert", GPNBerConfig)
AutoModel.register(GPNBerConfig, GPNBertModel)
AutoModelForMaskedLM.register(GPNBerConfig, GPNBertForMaskedLM)

AutoConfig.register("GPNGPT2", GPNGPT2Config)
AutoModel.register(GPNGPT2Config, GPNGPT2Model)
AutoModelForPreTraining.register(GPNGPT2Config, GPNGPT2LMHeadModel)
