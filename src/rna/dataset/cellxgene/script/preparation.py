...
#
parser = argparse.ArgumentParser(description="Build large-scale data in scBank format from a group of AnnData objects")
parser.add_argument(
    "--input-dir",
    type=str,
    required=True,
...
parser.add_argument(
    ...
    type=str,
    default="./data.scb",
    help="Directory to save scBank data, by default will make a directory named " "data.scb in the current directory",
)
parser.add_argument(
    "--include-files",
    type=str,
...
if args.metainfo is not None:
    ...
    include_obs = {
        f.stem: {"disease": metainfo[f.stem]["include_disease"]} for f in files if "include_disease" in metainfo[f.stem]
    }
...
for f in files:
    output_parquet_dt = output_dir / f"{f.stem}.scb" / f"{main_table_key}.datatable.parquet"
